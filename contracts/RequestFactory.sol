...
contract RequestFactory is RequestFactoryInterface, CloneFactory, Pausable {
     ...
     * @param _uintArgs [8]    -  txnData.callSmoke
     * @param _uintArgs [9]    -  txnData.callValue
     * @param _uintArgs [10]   -  txnData.smokePrice
        ...
        public payable returns (address)
    {
        ...
        if (!isValid.all()) {
            ...
            if (!isValid[4]) {
                emit ValidationError(uint8(Errors.CallSmokeTooHigh));
            }
            if (!isValid[5]) {
                emit ValidationError(uint8(Errors.EmptyToAddress));
            }

            // Try to return the 420coin sent with the message
