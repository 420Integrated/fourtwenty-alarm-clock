...
contract("Timestamp claiming", async (accounts) => {
  ...
  beforeEach(async () => {
    const { timestamp } = await config.web3.fourtwenty.getBlock("latest")
    const windowStart = timestamp + DAY

    txRecorder = await TransactionRecorder.new()

    const requiredDeposit = config.web3.utils.toMarley("25", "kmarley")
    ...
    await txRequest.initialize(
      ...
      [
        0, // fee
        config.web3.utils.toMarley("333", "willie"), // payment
        claimWindowSize,
        freezePeriod,
        reservedWindowSize,
        2, // temporal unit
  ...
  it("cannot claim before first claim stamp", async () => {
    ...
    expect(firstClaimStamp).to.be.above((await config.web3.fourtwenty.getBlock("latest")).timestamp)
  ...
  it("can claim at the first claim stamp", async () => {
    ...
    await waitUntilBlock(
      firstClaimStamp - (await config.web3.fourtwenty.getBlock("latest")).timestamp,
      1
    )
  ...
  it("can claim at the last claim stamp", async () => {
    ...
    expect(lastClaimStamp).to.be.above((await config.web3.fourtwenty.getBlock("latest")).timestamp)

    await waitUntilBlock(
      lastClaimStamp - (await config.web3.fourtwenty.getBlock("latest")).timestamp - 3,
      1
    )
  ...
  it("can not claim after the last claim stamp", async () => {
    ...
    const currentTimestamp = (await config.web3.fourtwenty.getBlock("latest")).timestamp
