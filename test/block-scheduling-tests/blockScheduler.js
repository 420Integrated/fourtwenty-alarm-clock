...
contract("Block scheduling", (accounts) => {
  ...
  const fee = 0
  const bounty = 0
  const requiredDeposit = config.web3.utils.toMarley("22", "kmarley")
  ...
  it("blockScheduler should arbitrarily accept payments sent to it", async () => {
    const balBefore = await config.web3.fourtwenty.getBalance(blockScheduler.address)
    ...
    const balAfter = await config.web3.fourtwenty.getBalance(blockScheduler.address)
    expect(balBefore < balAfter, "It sent 1000 marley correctly.").to.be.true
  })
  ...
  it("should do block scheduling with `schedule`", async () => {
    ...
    // Endowment is the minimum amount of 420coin that must be sent for the transaction
  ...
  // This test fails because the call smoke is too high
  it("should revert on invalid transaction", async () => {
    ...
    await blockScheduler
      .schedule(
        ...
        [
          4e20, // callSmoke is set way too high
          123454321, // callValue
          0, // windowSize
          windowStart,
          smokePrice,
        ...
        { from: User2, value: config.web3.utils.toMarley("10") }
      )
