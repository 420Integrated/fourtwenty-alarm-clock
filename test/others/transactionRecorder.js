...
contract("Test TransactionRecorder", (accounts) => {
  it("should send a transaction as specified", async () => {
    ...
    let lastCallSmoke = await txRecorder.lastCallSmoke()
    ...
    lastCallSmoke.toNumber().should.equal(0)
    ...
    lastCallSmoke = await txRecorder.lastCallSmoke()
    ...
    const callSmokeDelta = lastCallSmoke.toNumber() - 3000000
    expect(lastCallSmoke.toNumber() > 0, "Should have used some smoke.").to.be.true
    expect(callSmokeDelta < 10000, "But not too much smoke...").to.be.true
