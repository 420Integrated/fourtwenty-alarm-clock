...
contract("Request factory", async (accounts) => {
  ...
  const getWindowStart = async () => (await config.web3.fourtwenty.getBlockNumber()) + 20
  ...
  it("should create a request with provided properties", async () => {
    ...
    params.push(transactionRequest.smokePrice)
    ...
    const expectedCallData = fourtwentyUtil.bufferToHex(Buffer.from(transactionRequest.testCallData))
    ...
    expect(requestData.txData.callSmoke).to.equal(transactionRequest.callSmoke)
  ...
  it("should test request factory has too high call smoke validation error", async () => {
    const callSmoke = 8.8e8 // cannot be over smoke limit
    const { isValid } = await validate({ properties: { callSmoke } })
  ...
  it("should test null to address validation error", async () => {
    const to = NULL_ADDR // cannot be over smoke limit
