...
const computeEndowment = (bounty, fee, callSmoke, callValue, smokePrice) => (
  bounty + (fee * 2) + (callSmoke * smokePrice) + (180000 * smokePrice) + callValue
)
...
const parseRequestData = async (transactionRequest) => {
  ...
  return {
    ...
    txData: {
      callSmoke: data[2][11].toNumber(),
      callValue: data[2][12].toNumber(),
      smokePrice: data[2][13].toNumber(),
      toAddress: data[0][5],
    },
  }
}
  ...
  calcEndowment() {
    return (
      this.paymentData.bounty
            ...
            + (this.txData.callSmoke * this.txData.smokePrice)
            + (180000 * this.txData.smokePrice)
