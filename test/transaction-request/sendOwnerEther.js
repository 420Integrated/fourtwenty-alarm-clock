...
contract("TransactionRequestCore", async (accounts) => {
  const getBalance = async address => parseInt(await config.web3.fourtwenty.getBalance(address), 10)
  ...
  before(async () => {
    ...
    const funds = 10 ** 18 // 1 fourtwentycoin	
    const delay = 50
    const payout = 10 ** 17 // 0.1 fourtwentycoin
    const smokePrice = 10 ** 6
    ...
    const currentBlock = await config.web3.fourtwenty.getBlockNumber()
    ...
    await scheduledTransaction.execute({ from: accounts[2], smoke: 3000000, smokePrice })
  ...
  it("should fail to send remaining funds to the contract", async () => {
    ...
    // function in proxy contract which consumes more than 2300 smoke
    await proxy.sendOwnerFourtwentycoin(proxy.address)
  ...
  it("should fail to send remaining funds to the account when not owner", async () => {
    await proxy.sendOwnerFourtwentycoin(wallet, { from: accounts[2] })
  ...
  it("should fail to send remaining funds to null account", async () => {
    await proxy.sendOwnerFourtwentycoin('')
  ...
  it("should succeed to send remaining funds to the account", async () => {
    await proxy.sendOwnerFourtwentycoin(wallet)
