...
contract("Cancelling", async (accounts) => {
  ...
  it("tests CAN cancel before the claim window", async () => {
    ...
    expect(cancelAt).to.be.above(await config.web3.fourtwenty.getBlockNumber())
  ...
  it("tests CANNOT be cancelled if claimed and before the freeze window", async () => {
    ...
    const claimTx = await txRequest.claim({
      from: accounts[1],
      value: config.web3.utils.toMarley((2 * requestData.paymentData.bounty).toString()),
    })
  ...
  it("tests CANNOT cancel if was called", async () => {
    ...
    expect(executeAt).to.be.above(await config.web3.fourtwenty.getBlockNumber())
  ...
  it("tests CANNOT cancel if already cancelled", async () => {
    ...
    expect(cancelFirst).to.be.above(await config.web3.fourtwenty.getBlockNumber())
  ...
  it("tests accounting for pre-execution cancellation", async () => {
    ...
    // const balanceBeforeCancel = await config.web3.fourtwenty.getBalance(accounts[1])
    // const contractBalanceBefore = await config.web3.fourtwenty.getBalance(txRequest.address)
    ...
    // const balanceAfterCancel = await config.web3.fourtwenty.getBalance(accounts[1])
    // const contractBalanceAfter = await config.web3.fourtwenty.getBalance(txRequest.address)
  ...
  //     /// Sanity checks
  //     assert(cancelAt > await config.web3.fourtwenty.getBlockNumber())
  ...
  //     const beforeCancelBal = await config.web3.fourtwenty.getBalance(Owner)
  //     const beforeContractBal = await config.web3.fourtwenty.getBalance(transactionRequest.address)
  ...
  //     const afterCancelBal = await config.web3.fourtwenty.getBalance(Owner)
  //     const afterContractBal = await config.web3.fourtwenty.getBalance(transactionRequest.address)
