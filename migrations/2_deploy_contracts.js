...
NOW DEPLOYING THE FOURTWENTY ALARM CLOCK CONTRACTS...\n`)

  let balanceBefore;
  web3.fourtwenty.getBalance("0x21ec253c9186065f05fb3f541085a185f96a16ee", (err, res) => {
    balanceBefore = res;
  })

  deployer.deploy(MathLib, { smoke: 250000, overwrite })
    .then(() => deployer.deploy(IterTools, { smoke: 250000, overwrite }))
    .then(() => deployer.deploy(ExecutionLib, { smoke: 250000, overwrite }))
    .then(() => deployer.deploy(RequestMetaLib, { smoke: 250000, overwrite }))
    .then(() => deployer.deploy(SafeMath, { smoke: 250000, overwrite }))
    .then(() => {
      deployer.link(SafeMath, ClaimLib)

      return deployer.deploy(ClaimLib, { smoke: 250000, overwrite })
    })
    .then(() => {
      deployer.link(SafeMath, PaymentLib)

      return deployer.deploy(PaymentLib, { smoke: 250000, overwrite })
    })
    .then(() => {
      deployer.link(SafeMath, RequestScheduleLib)

      return deployer.deploy(RequestScheduleLib, { smoke: 250000, overwrite })
    })
    .then(() => {
      ...
      return deployer.deploy(RequestLib, { smoke: 3000000, overwrite })
    })
    .then(() => {
      ...
      return deployer.deploy(TransactionRequestCore, { smoke: 3000000, overwrite })
    })
    .then(() => {
    ...
    .then(() => {
      ...
      return deployer.deploy(BaseScheduler, { smoke: 1500000 })
    })
    .then(() => {
    ...
    .then(() => deployer.deploy(TransactionRecorder, { smoke: 750000 }))
    .then(() => {
      const contracts = {
      ...
      if (network && network !== "development") {
        ...
        let balanceAfter;
        web3.fourtwenty.getBalance("0x21ec253c9186065f05fb3f541085a185f96a16ee", (e, r) => {
          balanceAfter = r;
