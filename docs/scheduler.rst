...
* ``payment``: ``1000000 * tx.smokeprice`` set at the time of scheduling.
* ``fee``: ``10000 * tx.smokeprice`` or 1/100th of the default payment.
...
contract with different call signatures. (Prior versions of the 420AlarmClock had 2 API methods, we 
...
is executed. The recipient can be any valid 420coin address including both 
...
* ``_uintArgs[0]``: The ``callSmoke`` to be sent with the executing transaction.
* ``_uintArgs[1]``: The ``value`` in marley to be sent with the transaction.
...
* ``_uintArgs[4]``: The ``smokePrice`` that must be sent with the executing transaction.
...
When scheduling a transaction, you must provide sufficient 420coin to cover all
of the execution costs with some buffer to account for possible changes in the
network smoke price.  See :ref:`endowment` for more information on how to compute
the endowment.
