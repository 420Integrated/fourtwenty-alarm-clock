...
------
 ...
 - Recalibration of project for the 420coin blockchain protocols.
...
- Scheduled calls can now specify a required smoke amount.  This takes place of
  the ``suggestedSmoke`` api from 0.6.0
...
- The account management API has been removed.  Each call contract now manages
  it's own smoke money, the remainder of which is given back to the scheduler
  after the call is executed.
- All of the information that used to be stored about the call execution is now
  placed in event logs (smokeUsed, wasSuccessful, wasCalled, etc)
...
- Re-enable the right tree rotation in favor of removing three ``getLastX``
  function.  This is related to the pi-million smoke limit which is restricting
