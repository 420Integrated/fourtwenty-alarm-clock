...
This guide is inteded for smart contract developers that may want to use the 420AlarmClock services 
from within their own applications. Since all the functionality of the Alarm Clock 
is built into the 420coin blockchain via smart contracts, it can be accessed from other 
...
The first step is to establish how we will interact with the 420AlarmClock service's
...
The contract above is designed to lock away and then send to ``receiver`` whatever 420coin it is given for
...
* ``uint callSmoke``: The amount of smoke that will be sent with the transaction.
* ``uint callValue``: The amount of 420coin (in marley) that will be sent with the transaction.
...
* ``uint smokePrice``: The smoke price (in marley) which must be sent by the executing party to execute the transaction.
* ``uint fee``: The fee amount (in marley) included in the transaction for protocol maintainers.
* ``uint bounty``: The payment (in marley)included in the transaction to incentivse the executing arguments
* ``uint deposit``: (optional) Required amount of 420coin (in marley) to be staked by executing agents 

The ``0.1 420coin`` amount passed as value to ``schedule`` method pays for smoke, fee and bounty. The remaining amount of 420coin
...
In this example we are locking the sent 420coin in :class:`DelayedPayment` contract and using
...
payment, but the 420AlarmClock will work with arbritrary logic.  As the logic for a scheduled 
transaction increases, be sure to increase the required ``callSmoke`` in accordance.
Right now, the smoke limit for a transaction is somewhere in the ballpark of 8,000,000 
