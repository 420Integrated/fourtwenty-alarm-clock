...
* Unsigned Integers (``uint`` aka ``uint256``)
    ...
    *  ``_uintArgs[8]  => txnData.callSmoke``
    *  ``_uintArgs[9]  => txnData.callValue``
    *  ``_uintArgs[10] => txnData.smokePrice``
...
parameter ``endowment`` should be the amount in marley that will be sent during
contract creation.
...
and payment as well as smoke reimbursment.

The required minimum endowment can be computed as the sum of the following:

* ``callValue`` to provide the 420coin that will be sent with the transaction.
* ``2 * payment`` to pay for maximum possible payment
* ``2 * fee`` to pay for maximum possible fee
* ``callSmoke * txnData.smokePrice`` to pay for ``callSmoke``.
* ``180000 * txnData.smokePrice`` to pay for the smoke overhead involved in
...
Check #5: Call Smoke too high
^^^^^^^^^^^^^^^^^^^^^^^^^^^

* ``result[5]``

Check that the specified ``callSmoke`` value is not greater than the current
``smokeLimit - 140000`` where ``140000`` is the smoke overhead of request
execution.
