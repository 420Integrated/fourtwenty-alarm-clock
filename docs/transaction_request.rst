...
.. method:: TransactionRequest.Cancelled(uint rewardPayment, uint measuredSmokeConsumption)
...
* ``5 => InsufficientSmoke``
* ``6 => MismatchSmokePrice``


.. method:: TransactionRequest.Executed(uint payment, uint fee, uint measuredSmokeConsumption)
...
``measuredSmokeConsumption`` is the amount of smoke that was reimbursed which
should always be slightly greater than the actual smoke consumption.
...
* Unsigned 256 bit Integers (``uint`` aka ``uint256``)
    ...
    * ``uintValues[1]  => paymentData.anchorSmokePrice``
    ...
    * ``uintValues[12] => txnData.callSmoke``
    * ``uintValues[13] => txnData.callValue``
    * ``uintValues[14] => txnData.smokePrice``
...
.. attribute:: uint callValue
    ...
    The amount of 420coin, in marley, that will be sent with the transaction.

.. attribute:: uint callSmoke

    The amount of smoke that will be sent with the transaction.

.. attribute:: uint smokePrice 

    The smoke price required to send when executing the transaction.
...
.. attribute:: uint anchorSmokePrice

    The smoke price that was used during creation of this request.  This is used
    to incentivise the use of an adequately low smoke price during execution.

    See :ref:`smoke-multiplier` for more information on how this is used.


.. attribute:: uint payment

    The amount of 420coin in marley that will be paid to the account that executes
...
.. attribute:: uint paymentOwed
    ...
    The amount of 420coin in marley that is owed to the ``paymentBenefactor``. In
...
.. attribute:: uint fee
    ...
    The amount of 420coin, in marley, that will be sent to the `feeRecipient`
    upon execution.
...
.. attribute:: uint feeOwed
    ...
    The amount of 420coin in marley that is owed to the ``feeRecipient``. In
    most situations this will be zero at the end of execution, however, in the
...
.. attribute:: uint claimDeposit
    ...
    The amount of 420coin, in marley, that has been put down as a deposit towards
...
a call to buy from an ICO with the 420AlarmClock, they will need to proxy call the token contract 
after the execution in order to move the tokens they bought out of the TransactionRequest 
contract.


Retrieval of 420coin
------------------
...
Return any extra 420coin
^^^^^^^^^^^^^^^^^^^^^^

This function will send any exta 420coin in the contract that is not owed as a
